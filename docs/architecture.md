...
Located in [src/utils/paths.py](../src/utils/paths.py), this class centralizes path mapping and ensures strict XDG Base Directory specification compliance:
...
- Decouples file lookups from system-specific locations, enabling compatibility across Debian package and source installations.

### B. Configuration Manager (`ConfigManager`)
Located in [src/utils/config.py](../src/utils/config.py), this module loads, validates, and serializes user preferences and categories:
...
Located in [src/monitor.py](../src/monitor.py), this utilizes the `watchdog` framework to listen to file events in the monitored directory in real-time, executing transfers when new files arrive.

### D. Rule Engine (`RuleEngine`)
Located in [src/rules/engine.py](../src/rules/engine.py), it evaluates user-defined rules against file attributes (metadata, size, matching regex).
